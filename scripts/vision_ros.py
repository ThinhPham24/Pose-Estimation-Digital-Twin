...
from utils.similarity_2D import calculate_vgg19_pytorch_similarity, calculate_dinov2_pytorch_similarity
...
try:
    ...
    home_robot = [380.42, -94.67, 454.84, 159.13, 2.23, 170.08]
    while True:
        ...
        if object_infs is None:
            ...
            for pcd, img, cls  in object_infs:
                ...
                similarity_results = calculate_dinov2_pytorch_similarity(img, output_dir_cad)
                ...
                if similarity_results:
                    ...
                    if float(similarity_results[most_similar_image]) < threshold:
                        ...
                        if transformation is not None:
                            ...
                            # print("Inverse transformation matrix:", inv_transformation)
                            # print("Translate", transformation[:3,3])
                            # print("Rotation", transformation[:3,:3])
                            postue_inv = rotation2Euler_scale(inv_transformation)
                            # print("Pose camera inv", postue_inv )
                            pose_robot_inv = camera2robot(postue_inv , home_robot)
            ...
            for i, name in enumerate(obj_names):
                model_vis_path = f'/home/airlab/Desktop/DigitalTwin_PoseEstimation/data/ply_models/{name}.ply'
                mesh = trimesh.load_mesh(model_vis_path)
                model_point = mesh.sample(1024).astype(np.float32)
